    ...
    useEffect(() => {
        if (typeof window != 'undefined') {
            window.addEventListener('scroll', () => {
                if (window.scrollY > 750) setTheme('dark');
                else setTheme('light');
            });
        };
    });
    ...
    return (
        <header className={`header_${theme}`}>
            <div className={`header-container_${theme}`}>
                <Link href="/">
                    <svg onClick={scrollTop} className={`header-logo_${theme}`} width="56" height="38" viewBox="0 0 56 38" fill="none" xmlns="http://www.w3.org/2000/svg">
